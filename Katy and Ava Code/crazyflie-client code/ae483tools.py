...
def transform_data_mocap(raw_data):
    ...
    # Define parameters
    d_1 = 0.01158
    d_2 = 0.01823 # use the values found before 
    ...
    r = Rotation.from_euler('ZYX', [psi, theta, phi], degrees = False)
   # R_yaw = np.array([[np.cos(psi), -np.sin(psi), 0],
    ...
    for i in range(len(data['time'])):
        ...
        R_inQ_ofB = R_inQ_ofA @ R_inA_ofB               # <-- FIXME
        p_inQ_ofB = p_inQ_ofA + R_inQ_ofA @ p_inA_ofB # <-- FIXME

        # Pose of drone body frame in drone world frame
        R_inW_ofB = R_inW_ofQ @ R_inQ_ofB             # <-- FIXME
        p_inW_ofB =  p_inW_ofQ + R_inW_ofQ @ p_inQ_ofB  # <-- FIXME

        # Replace measurements of (x, y, z) and (phi, theta, psi) from mocap
        data['x'][i], data['y'][i], data['z'][i] = p_inW_ofB[0], p_inW_ofB[1], p_inW_ofB[2]               # <-- FIXME
        r= Rotation.from_matrix(R_inW_ofB)
        angles = r.as_euler('ZYX', False)
        data['yaw'][i], data['pitch'][i], data['roll'][i] = angles      # <-- FIXME
...
def sync_data_mocap(raw_data_mocap, t, z_drone,
                    do_transform=True, t_min_offset=0.5,
    ...
    for i, t_shift_bnds in enumerate(t_shifts):
